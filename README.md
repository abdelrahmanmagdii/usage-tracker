...
![UsageBar menu bar opens to a Codex quota window and an announced reset](docs/usagebar-demo.gif)
...
The Antigravity meter asks the running Antigravity app for quota. Sign in inside that app. UsageBar does not need `agy login`.

While the app is open, its language server already holds the Google session. UsageBar reads the 5-hour and weekly windows for Gemini models and for Claude/GPT models from that local server. The login stays on this Mac.

If the app is closed, UsageBar can use a still-valid access token from Keychain item `gemini` / `antigravity`, or from `~/.gemini/antigravity-cli/antigravity-oauth-token`. It does not refresh or write that token. Open the app again when the token expires.
