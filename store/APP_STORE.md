...
3. Settings is the large button at the bottom of the popover (tools, which window each meter follows, compact vs extended, alerts, launch at login). Right-click the menu bar icon for the same.

Temporary sandbox exceptions let the app read `~/.claude`, Cursor's Application Support DB, OpenCode `auth.json`, and spawn the user-installed `codex` binary (Homebrew `/opt/homebrew/bin/codex` and `/usr/local/bin/codex`). We do not write those locations.
